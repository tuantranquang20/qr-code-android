...
class CameraSource(private val graphicOverlay: GraphicOverlay) {
    ...
    private var facing: Int = CameraInfo.CAMERA_FACING_BACK
    ...
    fun changeZoom(scale: Float): Int {
        synchronized(processorLock) {
            if (camera == null) {
                return 0
            }
            ...
            parameters.zoom = (scale * maxZoom).roundToInt()
            camera!!.parameters = parameters
            return currentZoom
        }
    }
    ...
    fun switchCamera() {
        facing = if (facing == CameraInfo.CAMERA_FACING_FRONT) {
    ...
    private fun createCamera(): Camera {
        val camera = Camera.open(facing) ?: throw IOException("There is no back-facing camera.")
        val parameters = camera.parameters
    ...
    companion object {
         ...
         */
        private fun selectSizePair(camera: Camera): CameraSizePair? {
